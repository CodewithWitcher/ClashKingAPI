...
from utils.utils import fix_tag, gen_season_date
...
async def war_previous(clan_tag: str, timestamp_start: int = 0, timestamp_end: int = 9999999999, limit: int= 50, *, mongo: MongoClient):
    ...
    full_wars = await mongo.clan_wars.find({"$and" : [
    ...
    for war in full_wars:
        war_id = war.get("data").get("preparationStartTime")
        if war_id in found_ids:
            continue
        if "_response_retry" in war:
            del war["_response_retry"]
        new_wars.append(war.get("data"))
        found_ids.add(war_id)
...
@cache(expire=300)
@linkd.ext.fastapi.inject
async def war_previous_time(clan_tag: str, end_time: str, *, mongo: MongoClient):
    ...
    clan_tag = fix_tag(clan_tag)
    war = await mongo.clan_wars.find_one({"$and" : [{"$or" : [{"data.clan.tag" : clan_tag}, {"data.opponent.tag" : clan_tag}]},
...
@cache(expire=300)
@linkd.ext.fastapi.inject
async def basic_war_info(clan_tag: str, *, mongo: MongoClient):
    now = pend.now(tz=pend.UTC).timestamp() - 183600
    result = await mongo.clan_wars.find({"$and" : [{"clans" : fix_tag(clan_tag)}, {"custom_id": None}, {"endTime" : {"$gte" : now}}]}).sort({"endTime" : -1}).to_list(length=None)
...
@cache(expire=300)
@linkd.ext.fastapi.inject
async def cwl_group(clan_tag: str, *, mongo: MongoClient):
    clan_tag = fix_tag(clan_tag)
    season = gen_season_date()
    cwl_result = await mongo.cwl_groups.find_one({"$and" : [{"data.clans.tag" : clan_tag}, {"data.season" : season}]}, {"_id":0})
    return cwl_result
...
@cache(expire=300)
@linkd.ext.fastapi.inject
async def cwl(clan_tag: str, season: str, *, mongo: MongoClient):
    clan_tag = fix_tag(clan_tag)
    cwl_result = await mongo.cwl_groups.find_one({"$and" : [{"data.clans.tag" : clan_tag}, {"data.season" : season}]})

    if cwl_result is None:
        raise HTTPException(status_code=404, detail="No CWL Data Found")
    rounds = cwl_result.get("data").get("rounds")
    war_tags = []
    for cwl_round in rounds:
        for tag in cwl_round.get("warTags"):
            war_tags.append(tag)
    matching_wars = await mongo.clan_wars.find({"$and" : [{"data.tag" : {"$in" : war_tags}}, {"data.season" : season}]}).to_list(length=None)
    matching_wars = {w.get("data").get("tag") : w.get("data") for w in matching_wars}
    for r_count, cwl_round in enumerate(rounds):
        for count, tag in enumerate(cwl_round.get("warTags")):
...
def ranking_create(data: dict):
    ...
    for cwl_round in data.get("rounds"):
        for war in cwl_round.get("warTags"):
    ...
    sorted_list = sorted(star_list, key=lambda item: (item[2], item[3]), reverse=True)
    ...
    for item in sorted_list:
        ...
        result.append({
            ...
            "destruction": item[3],
            "rounds": {
                "won": rounds_won.get(clan_tag, 0),
                "tied": rounds_tied.get(clan_tag, 0),
                "lost": rounds_lost.get(clan_tag, 0)
            }
        })
    return result
